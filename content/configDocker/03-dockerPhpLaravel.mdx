...
Essa seção irá explicar como configurar o Docker Compose para a instalação
de um container que irá rodar o php 7.4 e mysql 5.7 já configurados.
O repositório oficial está neste link, [Docker](https://github.com/sport129/dockerFileLaravel).

### Glossário

  - app: Nome do nosso serviço, ele é um nome que você pode atribuir a gosto,
  ele será o nome caso precisemos linkar ele a algum outro serviço, é o nome que
  contém as informações daquele serviço.

  - build: Configuração que no momento de inicialização do docker, irá executar
  comandos para instalar serviços adicionais para aquele serviço, no caso, irá 
  instalar dependências somente para o serviço app.

  - ports: Definimos quais são as portas que irão sincronizar entre nossa máquina
  e o docker. A esquerda dos dois pontos é a porta que o nosso sistema deve liberar e a direita
  ...
  dividir os nossos diversos containers, de diversas outras configurações.
  ...
  - working_dir: Definimos o local de trabalho que o docker irá atuar no seu container, diretório dentro do container.

  - volumes: Onde definimos quais são os diretórios que o container irá escutar ou irá ter acesso. A esquerda
  do ponto e vírgula é definido qual o path que o docker irá utilizar no sistema do usuário e a
...
3.  Dentro da pasta dockerLaravel crie o arquivo docker-compose.yml
    ...
    Se você já possuir o VSCode instalado em sua máquina, basta executar o seguinte
...
1. Configurando a versão e dizendo os serviços
    ...
    no início do arquivo a seguinte informação:
    ...
    Estamos dizendo para o nosso docker que iremos utilizar a versão 3, e que iremos utilizar 
...
2.  Seviço php
    ...
    o container for inicializado ele deverá rodar um script para instalar dependências a este container.

    No dockerfile definimos o nome do arquivo que irá executar o nosso script.

    Para o funcionamento de debug do php, definimos duas variáveis de ambiente que o docker irá utilizar.
...
3.  Configurando local.ini
    ...
    3. Dizemos as configurações do `php` que o docker irá utilizar:
      ...
      wait_timeout = 3600
      ```
      Esse arquivo é como as configurações do `php.ini` quando instalamos
...
4.  Serviço Nginx
    ...
    3. Dentro de app.conf colocar as seguinte informações:
      ...
      Aqui fica definido quais configurações o nginx irá utilizar. A única coisa 
      a se atentar nessa configuração é a variável chamada root, pois ela é quem diz
...
6.  Linkar os serviços e definir o driver local
...
3. Colocar as seguintes informações
    ...
    O docker file irá executar esse script, para instalar serviços dentro do nosso serviço de php.
...
### Iniciando o Docker e Instalando as Dependências do Laravel

1. Vá até a pasta que está o docker
...
2. Execute o comando para instalar o container
    ...
    A instalação deve demorar alguns minutos, não se assuste.

3. Subir os serviços do docker, execute o seguinte comando
